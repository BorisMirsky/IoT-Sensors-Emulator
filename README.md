...
- в корне проекта выполнить `python -m iot_emulator start --config configs/test_device.yaml --speed 60` (это временный командный скрипт, по окончанию проекта будет более полный).


### Скрипт демонстрирует динамическое изменение поведения устройства по расписанию (через JSON-правила), без изменения кода. Температура автоматически растёт, достигает пика, затем падает — всё управляется файлом temperature_cycle.json:
- Загрузка конфигурации `configs/test_device.yaml` -> создание двух устройств (`test_sensor_1` - 5 правил поведения, `test_sensor_2` - нет правил поведения)
