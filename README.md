...
Например, разработана система мониторинга умного дома. Нужно проверить, как система обрабатывает поток данных от `n` датчиков температуры и влажности.

Результат: Нашли баги в обработке данных до того, как купили реальные датчики.

### 2. Нагрузочное тестирование MQTT-брокера

Хотим узнать, выдержитли выбранный MQTT-брокер `n` одновременно подключённых устройств, публикующих данные `m` раз в секунду.

Результат: Выясняли, что брокер падает при `n - 200` устройствах.


### 3.  Воспроизведение инцидентов

Например, накануне система мониторинга показала аномалию — резкий скачок температуры на 10°C за 2 минуты. Воспроизвести ситуацию вручную невозможно.
...
Результат (примерный): Нашли, что при потере пакетов обработчик падает с KeyError.
