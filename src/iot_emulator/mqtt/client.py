...
# Асинхронная обёртка над paho-mqtt. Позволяет подключаться, публиковать сообщения и подписываться на топики.
class MQTTClient:
    ...
    # Подключиться к MQTT брокеру. Возвращает True при успешном подключении.
    ...
    # Опубликовать сообщение в топик. Возвращает True при успешной публикации.
    async def publish(
        self,
        topic: str,
        payload: str,
